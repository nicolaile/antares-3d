...
- id: 23bf3392-55e2-4423-ac33-ff1721ebbd99
- displayName: Gradient map
- kind: effect
- description: Maps layer luminance to a configurable color gradient ramp with scatter, offset, repeat, and color-space blending controls.

Original user prompt:
Create a custom shader fill about gradient map using exact provided code.

Description:
A post-process effect that converts the underlying layer's luminance into a user-defined color gradient. The shader computes per-pixel sRGB luma, optionally dithers it with scatter noise, then maps the result through a multi-stop gradient ramp. Supports none/repeat/mirror tiling modes, adjustable repeat frequency, an offset slider, and sRGB/Linear/OKLab color-space interpolation. The shader reacts to mouse position (usesMouse: true) to drive the offset value from the pointer X coordinate.

Controls:
- Gradient (gradient) - multi-stop color ramp, default white-cyan-black
- Scatter (number slider 0-100%) - dithering noise amount
- Offset (mouse-driven, not a UI param) - Average of normalized mouse X and Y: offset = (mouseX/width + mouseY/height) / 2. Gives 0 at top-left, 1 at bottom-right, with equal contribution from both axes regardless of aspect ratio. Uses frame.mousePosition
- Repeat type (select: None/Repeat/Mirror) - tiling mode
- Repeat frequency (number slider 1-10) - how many times the gradient repeats
- Mix space (select: sRGB/Linear/OKLab) - color interpolation space
- Sensitivity (number slider 0-100%, step 1, default 100%) - scales how much the mouse X position affects offset; 100% = 1:1 tracking, 0% = no mouse influence

IMPORTANT: The user has provided the EXACT source code to use. Copy it verbatim into main.ts. Do not modify the WGSL, do not change the defineProperties block, do not alter any logic. Use the code exactly as provided in the user's message below.

## Exact source code to use
...
export function render(device: any, frame: any) {
    ...
    device.queue.writeBuffer(frame.state.uniformBuf, 0, new Float32Array([
        ...
        (frame.mousePosition.x / Math.max(1, width)), 0, 0, 0,
